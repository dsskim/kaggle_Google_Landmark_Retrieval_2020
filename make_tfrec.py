...
flags.DEFINE_string('train_directory', './input/', 'Training data directory.')
flags.DEFINE_string('test_directory', None,
                    '(Optional) Testing data directory. Required only if '
                    'test_csv_path is not None.')
flags.DEFINE_string('output_directory', './output/', 'Output data directory.')
flags.DEFINE_string('train_csv_path', '/tmp/train.csv',
                    'Training data csv file path.')
flags.DEFINE_string('train_clean_csv_path', './input/new_train.csv',
...
flags.DEFINE_string('test_csv_path', None,
                    ...
                    'generated')
flags.DEFINE_integer('num_shards', 4, 'Number of shards in output data.')
flags.DEFINE_boolean('generate_train_validation_splits', True,
                     '(Optional) Whether to split the train dataset into'
                     'TRAIN and VALIDATION splits.')
flags.DEFINE_float('validation_split_size', 0.1,
...
def _get_clean_train_image_files_and_labels(csv_path, image_dir):
  ...
  for _, row in df.iterrows():
    images[row['id']] = {}
    images[row['id']]['file_id'] = row['id']
    images[row['id']]['label'] = row['landmark_id']
...
def _write_tfrecord(output_prefix, image_paths, file_ids, labels):
  ...
  for shard in range(FLAGS.num_shards):
    output_file = os.path.join(
        ...
        '%s-%.5d-of-%.5d-%d.tfrec' % (output_prefix, shard, FLAGS.num_shards, spacing[shard + 1] - spacing[shard]))
